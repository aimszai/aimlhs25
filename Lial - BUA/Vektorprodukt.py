...
def calculate_vector_product(a, b):
    if a.shape != b.shape:
        print("Error: Vectors must be of the same dimension.")
        exit(1)
    if a.shape[0] != 3:
        print("Error: Vector product is only defined for 3-dimensional vectors.")
        exit(1)
    result = np.cross(a, b)
    return result
...
def main():
    ...
    result = calculate_vector_product(a, b)
    print(f"Vektorprodukt ist: {result}")
